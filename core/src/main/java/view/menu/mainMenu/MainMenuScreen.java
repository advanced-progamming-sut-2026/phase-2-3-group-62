...
public class MainMenuScreen implements Screen {
    ...
    private void buildUI() {
        ...
        bottomLeft.add(leaderboardContainer).padRight(14).bottom();

        Stack statusBadgeStack = new Stack();
        TextureRegion badgeBgRegion = game.getTextureBank().region("IMAGE_UI_STORE_GACHA_PINATA_EPIC_PULLS_TEXT_BG");
        if (badgeBgRegion != null) {
            Image badgeBgImg = new Image(new TextureRegionDrawable(badgeBgRegion));
            badgeBgImg.setScaling(Scaling.stretch);
            statusBadgeStack.add(badgeBgImg);
        }

        boolean isOnline = NetworkManager.getInstance().isConnected();

        TextureRegion wifiRegion = game.getTextureBank().region("IMAGE_UI_JOUST_WIFI_ICON");
        if (wifiRegion != null) {
            wifiIconImg = new Image(new TextureRegionDrawable(wifiRegion));
            wifiIconImg.setScaling(Scaling.fit);
            wifiIconImg.setColor(isOnline ? Color.GREEN : Color.RED);
        }

        statusLabel = new Label(isOnline ? "ONLINE" : "OFFLINE", skin, "big_outline");
        statusLabel.setFontScale(0.95f);
        statusLabel.setColor(isOnline ? Color.GREEN : Color.RED);
        statusLabel.setAlignment(Align.center);

        Table contentTable = new Table();
        if (wifiIconImg != null) {
            contentTable.add(wifiIconImg).size(30, 30).padRight(8).center();
        }
        contentTable.add(statusLabel).center();

        statusBadgeStack.add(contentTable);

        Table badgeWrapper = new Table();
        badgeWrapper.add(statusBadgeStack).size(165, 44).bottom();
        bottomLeft.add(badgeWrapper).padBottom(18).bottom();
    ...
    public void render(float delta) {
        ...
        if (networkCheckTimer >= 1.0f) {
            ...
            if (statusLabel != null) {
                ...
                statusLabel.setColor(online ? Color.GREEN : Color.RED);
            }
            if (wifiIconImg != null) {
                wifiIconImg.setColor(online ? Color.GREEN : Color.RED);
            }
        }
