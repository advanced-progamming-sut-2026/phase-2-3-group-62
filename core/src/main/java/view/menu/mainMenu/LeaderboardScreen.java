...
public class LeaderboardScreen implements Screen {
    ...
    private void refreshLeaderboard() {
        ...
        if (!isOnline) {
            Label offlineNotice = new Label("You are offline", skin, "big_outline");
            offlineNotice.setColor(Color.RED);
            offlineNotice.setFontScale(1.3f);
            offlineNotice.setAlignment(Align.center);
            tableContent.add(offlineNotice).expand().center().row();
            return;
        }
        ...
        if (sortedUsers.isEmpty()) {
            Label noDataLabel = new Label("No leaderboard data found on server.", skin);
            noDataLabel.setColor(Color.LIGHT_GRAY);
            tableContent.add(noDataLabel).colspan(8).padTop(30).center().row();
            return;
        }
