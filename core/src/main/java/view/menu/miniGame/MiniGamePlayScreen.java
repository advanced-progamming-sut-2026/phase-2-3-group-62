...
import view.game.mainGame.GamePlayHud;
import view.game.mainGame.GamePlayInputHandler;
import view.game.mainGame.GamePlayScreen;
import view.game.renderers.*;
import view.menu.playMenu.MiniGameSelectionScreen;
...
public class MiniGamePlayScreen implements Screen {
    ...
    private TextureRegion loadMiniGameBackground() {
        Game modelGame = gameController.getGame();
        MiniGame mg = modelGame != null ? modelGame.getActiveMiniGame() : null;

        if (mg instanceof Vasebreaker || (minigameName != null && minigameName.equalsIgnoreCase("Vasebreaker"))) {
            TextureRegion r = game.getTextureBank().region("IMAGE_BACKGROUNDS_ROMAN_BG_TEXTURE");
            if (r != null) return r;
        } else if (mg instanceof IZombie || (minigameName != null && minigameName.equalsIgnoreCase("IZombie"))) {
            TextureRegion r = game.getTextureBank().region("IMAGE_BACKGROUNDS_CARNIVAL_TEXTURE");
            if (r != null) return r;
        } else if (mg instanceof WallnutBowling || (minigameName != null && minigameName.equalsIgnoreCase("WallnutBowling"))) {
            TextureRegion r = game.getTextureBank().region("IMAGE_BACKGROUNDS_EIGHTIES_TEXTURE");
            if (r != null) return r;
        } else if (mg instanceof Beghoul || (minigameName != null && minigameName.equalsIgnoreCase("Beghoul"))) {
            TextureRegion r = game.getTextureBank().region("IMAGE_BACKGROUNDS_DARK_TEXTURE");
            if (r != null) return r;
        } else if (mg instanceof Zombotany || (minigameName != null && minigameName.equalsIgnoreCase("Zombotany"))) {
            TextureRegion r = game.getTextureBank().region("IMAGE_BACKGROUNDS_FUTURE_TEMP_TEXTURE");
            if (r != null) return r;
        }

        TextureRegion fallback = game.getTextureBank().region("IMAGE_BACKGROUNDS_BACKGROUND_LOD_BIRTHDAY_TEXTURE");
        if (fallback == null) fallback = game.getTextureBank().region("IMAGE_BACKGROUNDS_LAWN_DAY");
        return fallback;
    }
    ...
    public void show() {
        ...
        if (modelGame != null && modelGame.getActiveMiniGame() != null) {
            AudioManager.getInstance().playMiniGameMusic(modelGame.getActiveMiniGame());
        } else {
        ...
        bgRegion = loadMiniGameBackground();
