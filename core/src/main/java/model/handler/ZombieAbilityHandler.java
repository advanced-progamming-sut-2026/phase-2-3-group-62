...
public class ZombieAbilityHandler {
        ...
        if (name.equalsIgnoreCase("ZombieRa")) {
            ...
            if (zombie.getRaStealTimer() >= 20) {
                ...
                if (!game.getSuns().isEmpty()) {
                    ...
                    zombie.triggerCustomAnim("power_down", 1.2f);
        ...
        if (name.equalsIgnoreCase("ZombieTombRaiser")) {
            ...
            if (zombie.getTombraiserTimer() >= 100) {
                ...
                if (tile != null && tile.isEmpty() && tile.getType() == model.enums.TileType.GRASS) {
                    ...
                    zombie.triggerCustomAnim("power", 1.2f);
    ...
    public void processZombieDeathDrops(Zombie zombie, Game game) {
        Random r = new Random();
        int zCol = (int) Math.round(zombie.getX());
        int zRow = zombie.getY();

        if (zCol < 0) zCol = 0;
        if (zCol >= game.getBoard().getColumns()) zCol = game.getBoard().getColumns() - 1;

        if (zombie.isGlowing()) {
            boolean alreadyHasDroppedPf = false;
            if (game.getDroppedItems() != null) {
                for (DroppedItem item : game.getDroppedItems()) {
                    if (item.getType() == DroppedItem.ItemType.PLANT_FOOD) {
                        alreadyHasDroppedPf = true;
                        break;
                    }
                }
            }

            if (!alreadyHasDroppedPf && game.getPlantFoodCount() < 3) {
                game.addDroppedItem(new DroppedItem(DroppedItem.ItemType.PLANT_FOOD, zRow, zCol, (float) zombie.getX(), zRow));
                game.getGameLogMessages().add("The glowing zombie dropped a plant food at (" + zCol + ", " + zRow + ")!");
            }
            return;
        }
        ...
        if (r.nextInt(100) < 6) {
            ...
            if (roll < 60) {
                game.addDroppedItem(new DroppedItem(DroppedItem.ItemType.COIN, zRow, zCol, (float) zombie.getX(), zRow));
                game.getGameLogMessages().add("A zombie dropped a coin at (" + zCol + ", " + zRow + ")!");
            } else if (roll < 85) {
                game.addDroppedItem(new DroppedItem(DroppedItem.ItemType.DIAMOND, zRow, zCol, (float) zombie.getX(), zRow));
                game.getGameLogMessages().add("A zombie dropped a diamond at (" + zCol + ", " + zRow + ")!");
            } else {
                game.addDroppedItem(new DroppedItem(DroppedItem.ItemType.POT, zRow, zCol, (float) zombie.getX(), zRow));
                game.getGameLogMessages().add("A zombie dropped a pot at (" + zCol + ", " + zRow + ")!");
            }
        }
    }
